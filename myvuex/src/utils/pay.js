var $ = require('zepto');
import chelunJSBridge from './JSBridge';
...
export default {
    ...
    wxPay: function(data){
        ...
        param += '&c=order&v=pay&app=AgentNumberPlate';
     ...
     */
    appPay: function(data){
        let version = this.getCookie('chelun_appVersion'),
            app = this.getCookie('chelun_appName').toLowerCase(),
            os = this.getCookie('chelun_osType'),
            channel = data.payInfo.channels;
        if (version == '6.2.8' && os == 'android' && app == 'queryviolations'){
            channel = 'alipay';
        }
        var pay_param = {
            price: data.payMoney,
     ...
     */
    h5Pay: function(data){
        var self = this, pay_way = '',channels = data.payInfo.channels;
        if (channels.length) {
        ...
        if (!this.exist && pay_way){
            ...
            this.$deposit.text(this.deposit);
            this.show();
        }
            ...
            channels.forEach(function (item, index) {
                if (item == 'alipay'){
                    ...
                    self.$payAli.off('click').on("click", function(){
                        $.get(self.domain+'/api.php?platform=wap&channel=alipay&ac_token='+ac_token+'&order_id='+order_id+'&c=order&v=pay&' +
                            'app=AgentNumberPlate&serial_number='+serial_number+'&callback_url='+encodeURIComponent(self.callback_url),function(res){
                    ...
                    self.$payBd.off('click').on("click", function(){
                        $.get(self.domain+'/api.php?platform=wap&channel=baidu&ac_token='+ac_token+'&order_id='+order_id+'&c=order&v=pay&' +
                            'app=AgentNumberPlate&serial_number='+serial_number+'&callback_url='+encodeURIComponent(self.callback_url),function(res){
                            window.location = res.data.url;
                        }, 'json');
                    });
    ...
    loadImg: function(){
        var ali = new Image(), bd = new Image(), fq = new Image();
